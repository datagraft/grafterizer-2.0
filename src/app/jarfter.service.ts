...
export class JarfterService {
  ...
  generateClojure(transformation, isCsv, asiaEnrpointUrl) {
    ...
    var clojure = '(ns grafterizer.transformation (:require [grafter.tabular :refer :all] [clojure.string :refer [capitalize lower-case upper-case trim trim-newline triml trimr blank?]] [tabular_functions.datatypes :as datatypes] [tabular_functions.pipeline :as new-tabular] [clj-http.client :as client] [grafter.rdf :refer [prefixer s add]] [grafter.rdf.templater :refer [graph]] [grafter.rdf.formats :refer :all] [grafter.vocabularies.rdf :refer :all] [grafter.vocabularies.qb :refer :all] [grafter.vocabularies.sdmx-measure :refer :all] [grafter.vocabularies.sdmx-attribute :refer :all] [grafter.vocabularies.skos :refer :all] [grafter.vocabularies.foaf :refer :all] [grafter.vocabularies.owl :refer :all] [grafter.vocabularies.dcterms :refer :all] [clj-time.format] [grafter.rdf.io :as ses] [grafter.tabular.common :refer [read-dataset*]] ) (:import [gov.nasa.worldwind.geom.coords.UTMCoord] [org.openrdf.model.impl.URIImpl] [it.unimib.disco.asia.ASIAHashtableClient] [it.unimib.disco.asia.ASIA4JFactory] [it.unimib.disco.asia.ASIA4J] ) )'

    // pipeline and graft
    clojure += generateClojure.fromTransformation(transformation, (asiaEnrpointUrl === 'no-asia' ? false : true), asiaEnrpointUrl);
   ...
   */
  public getTransformationJar(transformationObj: string, asiaEnrpointUrl: string): Promise<any> {
    ...
    const clojure = this.generateClojure(transformationObj, false, asiaEnrpointUrl);
