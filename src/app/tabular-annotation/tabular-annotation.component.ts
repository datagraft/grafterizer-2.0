    ...
    if (reconciliationServiceId || this.transformationObj.isColumnResultOfExtension(currentHeader)) {
      if (annotation) { // to check if it's a date column
        if (annotation instanceof transformationDataModel.URINodeAnnotation) {
          const shortTypes = annotation.columnTypes.map((type: transformationDataModel.ConstantURI) => {
            const uri = this.transformationObj.getConstantURINodeFullyQualifiedName(type);
            return uri.substr(UrlUtils.getNamespaceFromURL(new URL(uri)).length);
          });
          ...
          const shortType = type.substr(UrlUtils.getNamespaceFromURL(new URL(this.transformationObj.getConstantURINodeFullyQualifiedName(type))).length);
